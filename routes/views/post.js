...
exports = module.exports = function (req, res) {
	...
	view.on('init', function (next) {
		PostComment.model.find()
			...
			.exec(function (err, comments) {
				...
				if (!comments) return res.notfound('Post Solution not found');
	...
	view.on('post', { action: 'comment.create' }, function (next) {
		...
		updater.process(req.body, {
			...
			if (err) {
				...
				req.flash('success', 'Your Solution was added.');
	...
	view.on('get', { remove: 'comment' }, function (next) {
		...
		if (!req.user) {
			req.flash('error', 'You must be signed in to delete a Solution.');
			return next();
		}
			...
			.exec(function (err, comment) {
				if (err) {
					if (err.name === 'CastError') {
						req.flash('error', 'The Solution ' + req.query.comment + ' could not be found.');
						return next();
					}
					return res.err(err);
				}
				...
				if (comment.author != req.user.id) {
					req.flash('error', 'Sorry, you must be the author of a Solution to delete it.');
					return next();
				}
				comment.commentState = 'archived';
				comment.save(function (err) {
					if (err) return res.err(err);
					req.flash('success', 'Your Solution has been deleted.');
